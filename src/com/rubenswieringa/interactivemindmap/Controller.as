package com.rubenswieringa.interactivemindmap {
	...
	public class Controller extends EventDispatcher {
		 ...
		 */
		public static function boot (springGraph:SpringGraph, url:String=""):void {
			if (Controller.booted){
				throw new IllegalOperationError("Controller has already been booted");
			}
			...
			// load:
			Model.instance.addEventListener(Model.READY, Controller.instance.onModelReady);
			if (url != ""){
				Model.instance.load(url);
			}
		}
		
		/**
		 * Executes build() when Model has finished loading.
		 */
		...
		public function build (id:String=""):void {
			// default:
			if (id == ""){
				id = Model.instance.data.words.word[0].@id;
			}
		...
		private function addItemLinks (item:Item):void {
			...
			var newItem:Item;
			var links:XMLList = Model.instance.data.links.link.(@a == item.id || @b == item.id);
			var link:XML;
			var linkID:String;
			var linkStyle:Object;
			var xml:XML;
	...
	// STATIC METHODS ---------------------------------------------------------------------------------------------
		...
		public static function getXML (id:String):XML {
			return Model.instance.data.words..word.(@id == id)[0] as XML;
		}
